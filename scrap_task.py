...
class QuoteScrapper():
    ...
    def fetch_data(self, url_path: str, category: str):
        """ """
        page: bool = True
        page_count = 1
        temp_cateory = []
        ...
        while page:
            curr_url: str
            try:
                curr_url = f"{ self.URL }{ url_path }page/{ page_count }/"
            ...
            for i in soup.findAll("div", {"class": "quote"}):
                ...
                self.fetch_author_info(self.URL + a["href"])
...
if __name__ == "__main__":
    ...
    quotes_scrapper.top_ten()

    for i, j in enumerate(quotes_scrapper.CATEGORIES_URLS):
        quotes_scrapper.fetch_data(j, quotes_scrapper.CATEGORY_NAMES[i])
